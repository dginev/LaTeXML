...
sub getopt_specification {
  ...
  my $spec      = {
    ...
    #       By {format} we mean "output_format" and
    #       by {type} we mean "input_format"!
    #       by {whatsin} we mean "input_packaging"
    #       by {whatsout} we mean "output_packaging"
...
sub read {
  ...
  # latexmlpost emulation. this is a mandatory override:
  # Post-processing is always ON, core is OFF, for XML inputs.
  if ($$opts{source} && ($$opts{source} =~ /$is_xml/)) {
    $$opts{post} = 1;
    $$opts{type} = 'xml'; }
  # The default fallback is archive,
  if (!$$opts{whatsin}) {
...
sub _prepare_options {
  ...
  if ($$opts{post}) {    # No need to bother if we're not post-processing
    ...
    # TODO: Maybe just remove the option entirely at this point?
    if (defined $$opts{parallelmath}) {
